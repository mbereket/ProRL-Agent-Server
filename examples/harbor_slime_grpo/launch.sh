...
Rollout:   ${ROLLOUT_BATCH_SIZE} prompts x ${N_SAMPLES_PER_PROMPT} samples, ${NUM_STEPS:+${NUM_STEPS} steps}${NUM_EPOCH:+${NUM_EPOCH} epoch(s)}; prompt/response caps ${ROLLOUT_MAX_PROMPT_LEN}/${ROLLOUT_MAX_RESPONSE_LEN}, sglang ctx ${SGLANG_CONTEXT_LENGTH}
